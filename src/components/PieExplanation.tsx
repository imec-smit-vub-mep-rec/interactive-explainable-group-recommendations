...
export default function PieExplanation({
  ...
  const getPieData = (restaurantIndex: number) => {
    ...
    switch (strategy) {
      case 'LMS': // Least Misery Strategy - only show lowest rating
        ...
        const lowestRaters = restaurantRatings.filter(r => r.rating === minRating);
  ...
  return (
    <div className="bg-white border border-gray-300 rounded-lg p-6">
      <div className="mb-4">
        <h3 className="text-lg font-semibold mb-2">Pie Chart Explanation</h3>
        <p className="text-sm text-gray-600 mb-4">
          Each pie chart represents a restaurant. Chart size reflects the group score. 
          Click on slices to change ratings (1-5 cycle).
        </p>
      </div>
