...
export default function OrderedListExplanation({
  ...
  const restaurantsWithRankings = rankedRestaurants.map((restaurant, index) => {
    ...
    if (index > 0 && restaurant.score === rankedRestaurants[index - 1].score) {
      ...
      const firstIndexWithSameScore = rankedRestaurants.findIndex(r => r.score === restaurant.score);
  ...
  const getStrategyExplanation = (restaurant: typeof rankedRestaurants[0]) => {
    const restaurantRatings = ratings.map(personRatings => personRatings[restaurant.originalIndex]);
    
    switch (strategy) {
      case 'LMS': {
        const minRating = Math.min(...restaurantRatings);
        const usersWithLowerRating = restaurantRatings.filter(rating => rating === minRating).length;
        return `The lowest rating is ${minRating}.`;
      }
      case 'ADD': {
        const totalRating = restaurantRatings.reduce((sum, rating) => sum + rating, 0);
        return `The total rating is ${totalRating}.`;
      }
      case 'APP': {
        const approvalVotes = restaurantRatings.filter(rating => rating >= 4).length;
