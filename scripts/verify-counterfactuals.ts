...
function strategyForScenarioType(t: string): Strategy {
  switch (t) {
    case "add":
      return "ADD";
    case "lms":
...
function recommendedRestaurants(
  ratings: number[][],
  visitedRestaurantIndices0: number[],
  strategy: Strategy
): number[] {
  const visited = new Set<number>(visitedRestaurantIndices0);
  const candidates: Array<{ idx: number; score: number }> = [];
  for (let i = 0; i < 10; i++) {
    if (visited.has(i)) continue;
    candidates.push({
      idx: i,
      score: computeRestaurantScore(ratings, i, strategy),
    });
  }
...
function parseModifications(choiceText: string): Array<{
  rest: number; // 1-based
  person: string;
  value: number;
}> {
  ...
  const re =
    /^Change\s+Rest\s+(\d+)\s+for\s+([A-Za-z]+)\s+to\s+(\d+)\s+and\s+Rest\s+(\d+)\s+for\s+([A-Za-z]+)\s+to\s+(\d+)\.?$/;
...
function isChangeFromSuggested(nextRecommended: number[], suggested: number): boolean {
  return !(nextRecommended.length === 1 && nextRecommended[0] === suggested);
}

function formatChoiceText(mods: Array<{ rest: number; person: string; value: number }>): string {
  const [a, b] = mods;
...
function proposeNameAssignmentsForScenario(input: {
  ...
  strategy: Strategy;
  suggested: number;
  choices: Array<{ id: string; text: string; isCorrectAnswer?: boolean }>;
}): null | Record<string, { mods: Array<{ rest: number; person: string; value: number }>; text: string }> {
  ...
  const assigned: string[][] = baseMods.map((mods) => mods.map((m) => m.person));

  const slots: Array<{ choiceIdx: number; modIdx: number }> = [];
  for (let ci = 0; ci < baseMods.length; ci++) {
    for (let mi = 0; mi < baseMods[ci].length; mi++) slots.push({ choiceIdx: ci, modIdx: mi });
  }
  ...
  function checkAll(): boolean {
    for (let ci = 0; ci < baseMods.length; ci++) {
      ...
      const nextRecommended = recommendedRestaurants(
        nextRatings,
        input.previous_visits,
        input.strategy
      );
      const changed = isChangeFromSuggested(nextRecommended, input.suggested);
      if (changed !== desired[ci]) return false;
    }
    return true;
  }

  function dfs(slotIdx: number): boolean {
    if (slotIdx === slots.length) return checkAll();
    ...
    for (const name of CANONICAL_NAMES) {
      ...
      if (modIdx === 1) {
        ...
        const changed = isChangeFromSuggested(nextRecommended, input.suggested);
        if (changed !== desired[choiceIdx]) continue;
      }
  ...
  const out: Record<
    string,
    { mods: Array<{ rest: number; person: string; value: number }>; text: string }
  > = {};
...
function proposeMinimalFixForScenario(input: {
  ...
  if (mismatchedChoiceIdxs.length !== 1) return null;
  ...
  let best: null | { names: string[]; texts: Record<string, string>; distance: number } = null;

  for (const nameA of CANONICAL_NAMES) {
    for (const nameB of CANONICAL_NAMES) {
      const trialMods = baseMods.map((mods, ci) =>
        mods.map((m) => ({ ...m }))
      );
      ...
      const distance =
        (nameA === originalNames[0] ? 0 : 1) + (nameB === originalNames[1] ? 0 : 1);
...
function applyModifications(
  ratings: number[][],
  mods: Array<{ rest: number; person: string; value: number }>
): number[][] {
...
function parseSuggestedRestaurant(questionText: string): number {
  // "... system suggests Rest 4. Which ..."
  const m = questionText.match(/system suggests Rest\s+(\d+)/);
  if (!m) throw new Error(`Could not find "system suggests Rest N" in: ${questionText}`);
  return Number(m[1]);
}

function main() {
  const counterfactualScenarios = scenarios.filter((s) =>
    s.questions?.some((q) => q.task === "counterfactual")
  );
  ...
  for (const s of counterfactualScenarios) {
    ...
    if (!baseRecommended.includes(suggested)) {
      failures.push(
        `${s.id}: question says suggested Rest ${suggested}, but computed recommended is [${baseRecommended.join(
          ", "
        )}] under ${strategy}`
      );
    }
    ...
    for (const choice of q.choices) {
      ...
      if (changed !== markedCorrect) {
        failures.push(
          `${s.id}/${choice.id}: markedCorrect=${markedCorrect} but changed=${changed}. suggested=${suggested}, next=[${nextRecommended.join(
            ", "
          )}] text="${choice.text}"`
        );
      }
  ...
  if (failures.length) {
    ...
    console.log(
      `Counterfactual verification PASSED for ${counterfactualScenarios.length} scenarios.`
    );
  }
}
